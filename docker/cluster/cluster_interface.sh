...
submit_job() {
    ...
    ssh $CLUSTER_LOGIN "cd $CLUSTER_PROJECT_DIR && bash $CLUSTER_PROJECT_DIR/isaaclab_ext/docker/cluster/$job_script_file \"$CLUSTER_PROJECT_DIR\" \"isaac-lab-ext-$suffix\" ${@}"
}
...
case $command in
    push)
        ...
        # clear old exports for selected suffix
        rm -rf /$SCRIPT_DIR/exports/isaac-lab-ext-$suffix*
        ...
        # Set a temporary directory for the build to avoid "file name too long" errors.
        export APPTAINER_TMPDIR="/tmp/apptainer-build-$(id -u)"
        mkdir -p "$APPTAINER_TMPDIR"
        # Change to the temporary directory to perform the build
        pushd "$APPTAINER_TMPDIR" > /dev/null
        APPTAINER_NOHTTPS=1 apptainer build --sandbox --fakeroot isaac-lab-ext-$suffix.sif docker-daemon://isaac-lab-ext-$suffix:latest
        # Return to the original directory
        popd > /dev/null
        # tar image (faster to send single file as opposed to directory with many files)
        tar -cvf /$SCRIPT_DIR/exports/isaac-lab-ext-$suffix.tar -C "$APPTAINER_TMPDIR" isaac-lab-ext-$suffix.sif
        ...
        scp $SCRIPT_DIR/exports/isaac-lab-ext-$suffix.tar $CLUSTER_LOGIN:$CLUSTER_SIF_PATH/isaac-lab-ext-$suffix.tar
        ;;
    job)
        if [ $# -ge 1 ]; then
        ...
        # Append current date and time to CLUSTER_PROJECT_DIR to create unique directories
        CLUSTER_PROJECT_DIR="${CLUSTER_PROJECT_DIR}_${current_datetime}"
        # Check if singularity image exists on the remote host
        check_singularity_image_exists isaac-lab-ext-$suffix
        # make sure target directories exist in a single SSH login
        ssh $CLUSTER_LOGIN "mkdir -p $CLUSTER_PROJECT_DIR $CLUSTER_PROJECT_DIR/isaaclab $CLUSTER_PROJECT_DIR/isaaclab_ext"
        # Sync Isaac Lab code
        echo "[INFO] Syncing Isaac Lab code..."
        rsync -rh  --exclude="*.git*" --exclude="logs" --exclude="outputs" --exclude="wandb" --filter=':- .dockerignore'  $ISAACLAB_PATH/ $CLUSTER_LOGIN:$CLUSTER_PROJECT_DIR/isaaclab
        # Sync Isaac Lab Extension code
        echo "[INFO] Syncing Isaac Lab Extension code..."
        rsync -rh  --exclude="*.git*" --exclude="logs" --exclude="outputs" --exclude="wandb" --filter=':- .dockerignore'  /$SCRIPT_DIR/../.. $CLUSTER_LOGIN:$CLUSTER_PROJECT_DIR/isaaclab_ext
        # execute job script
    ...
    cleanup)
        ...
        # source env file to get cluster login and path information
        source $SCRIPT_DIR/.env.cluster
        # Get the base project directory path and its parent
        base_project_dir_name=$(basename "$CLUSTER_PROJECT_DIR")
        parent_dir=$(dirname "$CLUSTER_PROJECT_DIR")

        echo "[INFO] Deleting directories matching '${base_project_dir_name}_*' in '$parent_dir' on '$CLUSTER_LOGIN'..."
        # Use ssh to find and delete the directories matching the timestamped pattern
        ssh $CLUSTER_LOGIN "find '$parent_dir' -mindepth 1 -maxdepth 1 -type d -name '${base_project_dir_name}_*' -exec rm -rf {} +"
